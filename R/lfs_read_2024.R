#' Read LFS 2024 quarters
#'
#' Reads and performs basic cleaning on the Labour Force Survey 2024 quarters.
#'
...
lfs_read_2024 <- function(
  ...
  cat(crayon::yellow("Reading LFS 2024:"))

  ###### Read in each quarter
  cat(crayon::green("\tJan - Mar..."))
  data.q1 <- data.table::fread(
    paste0(path,"/lfsp_jm24_eul_pwt24.tab"), showProgress = FALSE,
    na.strings = c("NA", "", "-1", "-2", "-6", "-7", "-8", "-9", "-90", "-90.0", "N/A")
  )

  cat(crayon::green("\tApr - Jun..."))
  data.q2 <- data.table::fread(
    paste0(path,"/lfsp_aj24_eul_pwt24.tab"), showProgress = FALSE,
    na.strings = c("NA", "", "-1", "-2", "-6", "-7", "-8", "-9", "-90", "-90.0", "N/A")
  )

  cat(crayon::green("\tJul - Sep..."))
  data.q3 <- data.table::fread(
    paste0(path,"/lfsp_js24_eul_pwt24.tab"), showProgress = FALSE,
    na.strings = c("NA", "", "-1", "-2", "-6", "-7", "-8", "-9", "-90", "-90.0", "N/A")
  )
  ...
  for (l in c(1:4)) {
    ...
    weights_vars     <- Hmisc::Cs(pwt24,piwt23)
