#' Read LFS 2011 quarters
#'
#' Reads and performs basic cleaning on the Labour Force Survey 2011 quarters.
#'
...
lfs_read_2011 <- function(
  ...
  cat(crayon::yellow("Reading LFS 2011:"))

  ###### Read in each quarter
  cat(crayon::green("\tJan - Mar..."))
  data.q1 <- data.table::fread(
    paste0(path ,"/lfsp_jm11_end_user.tab"), showProgress = FALSE,
    na.strings = c("NA", "", "-1", "-2", "-6", "-7", "-8", "-9", "-90", "-90.0", "N/A")
  )

  cat(crayon::green("\tApr - Jun..."))
  data.q2 <- data.table::fread(
    paste0(path ,"/lfsp_aj11_end_user.tab"), showProgress = FALSE,
    na.strings = c("NA", "", "-1", "-2", "-6", "-7", "-8", "-9", "-90", "-90.0", "N/A")
  )

  cat(crayon::green("\tJul - Sep..."))
  data.q3 <- data.table::fread(
    paste0(path ,"/lfsp_js11_end_user_pwt18.tab"), showProgress = FALSE,
    na.strings = c("NA", "", "-1", "-2", "-6", "-7", "-8", "-9", "-90", "-90.0", "N/A")
  )

  cat(crayon::green("\tOct - Dec..."))
  data.q4 <- data.table::fread(
    paste0(path ,"/lfsp_od11_end_user_pwt18.tab"), showProgress = FALSE,
    na.strings = c("NA", "", "-1", "-2", "-6", "-7", "-8", "-9", "-90", "-90.0", "N/A")
  )
  ...
  for (l in c(1)) {
    ...
    demographic_vars <- Hmisc::Cs(age,sex,govtof,ethuk8,marsta,fdpch16)
    education_vars   <- Hmisc::Cs(edage,hiqul11d,hiqual11,bte11,sctvc11,gnvq11,nvq11,rsa11,cag11,numol5,numal,numas,hst,advhst,
    ...
    data.table::setnames(data, c("refwkm", "pwt14","piwt14","bte11","sctvc11","gnvq11","nvq11","rsa11","cag11","numol5",
  ...
  for (l in c(2)) {
    ...
    education_vars   <- Hmisc::Cs(hiqul11d,hiqual11,bte11,sctvc11,gnvq11,nvq11,rsa11,cag11,numol5,numal,numas,hst,advhst,
                                  qual_1,qual_2,qual_3,qual_4,qual_5,qual_6,qual_7,qual_8,qual_9,qual_10,qual_11,qual_12,qual_13,qual_14,
                                  qual_15,qual_16,qual_17,qual_18,qual_19,qual_20,qual_21,qual_22,qual_23,qual_24,qual_25,qual_26,qual_27,
                                  qual_28,qual_29,qual_30,qual_31)
    health_vars      <- Hmisc::Cs(health,discurr)
