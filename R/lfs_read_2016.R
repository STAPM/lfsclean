#' Read LFS 2016 quarters
#'
#' Reads and performs basic cleaning on the Labour Force Survey 2016 quarters.
#'
...
lfs_read_2016 <- function(
  ...
  data.q1 <- data.table::fread(
    paste0(path ,"/lfsp_jm16_eul.tab"), showProgress = FALSE,
    na.strings = c("NA", "", "-1", "-2", "-6", "-7", "-8", "-9", "-90", "-90.0", "N/A")
  )

  cat(crayon::green("\tApr - Jun..."))
  data.q2 <- data.table::fread(
    paste0(path ,"/lfsp_aj16_eul.tab"), showProgress = FALSE,
    na.strings = c("NA", "", "-1", "-2", "-6", "-7", "-8", "-9", "-90", "-90.0", "N/A")
  )
  ...
  data.q4 <- data.table::fread(
    paste0(path ,"/lfsp_od16_eul.tab"), showProgress = FALSE,
    na.strings = c("NA", "", "-1", "-2", "-6", "-7", "-8", "-9", "-90", "-90.0", "N/A")
  )
