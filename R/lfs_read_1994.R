#' Read LFS 1994 quarters
#'
#' Reads and performs basic cleaning on the Labour Force Survey 1994 quarters.
#'
...
lfs_read_1994 <- function(
  ...
  for (l in c(1)) {
    ...
    education_vars   <- Hmisc::Cs(edage,hiquap,hiquapd)
    work_vars        <- Hmisc::Cs(inecacr,grsswk,ftptwk,ttachr,ttushr,publicr,indm92m,indd92m,inds92m,socmain,socminm)
  ...
  for (l in c(2,3,4)) {
    ...
    weights_vars     <- Hmisc::Cs(pwt07,piwt07)
    demographic_vars <- Hmisc::Cs(age,sex,govtor,ethcen,marcon)
    education_vars   <- Hmisc::Cs(hiquap,hiquapd,qual0,qual1,qual2,qualch,numal,nvqsvq,numol)
