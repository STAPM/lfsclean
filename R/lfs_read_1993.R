#' Read LFS 1993 quarters
#'
#' Reads and performs basic cleaning on the Labour Force Survey 1993 quarters.
#'
...
lfs_read_1993 <- function(
  ...
  for (l in c(1)) {
    ...
    demographic_vars <- Hmisc::Cs(age,sex,govtor,ethcen,marcon)
    education_vars   <- Hmisc::Cs(edage,qual0,qual1,qual2,qualch)
    work_vars        <- Hmisc::Cs(inecacr,grsswk,ftptwk,ttachr,ttushr,mpnor,socmain,socminm)
  ...
  for (l in c(2,3)) {
    ...
    weights_vars     <- Hmisc::Cs(pwt07,piwt07)
    demographic_vars <- Hmisc::Cs(age,sex,govtor,ethcen,marcon)
    education_vars   <- Hmisc::Cs(qual0,qual1,qual2,qualch,numal,nvqsvq,numol,nvqlev)
    work_vars        <- Hmisc::Cs(inecacr,grsswk,ftptwk,ttachr,ttushr,mpnor,socmain,socminm)
    other_vars       <- Hmisc::Cs(refwkm,thiswv)
  ...
  for (l in c(4)) {
    ...
    weights_vars     <- Hmisc::Cs(pwt07,piwt07)
    demographic_vars <- Hmisc::Cs(age,sex,govtor,ethcen,marcon)
    education_vars   <- Hmisc::Cs(hiquap,hiquapd,qual0,qual1,qual2,qualch,numal,nvqsvq,numol,nvqlev)
    work_vars        <- Hmisc::Cs(inecacr,grsswk,ftptwk,ttachr,ttushr,mpnor,socmain,socminm)
    other_vars       <- Hmisc::Cs(refwkm,thiswv)
