#' Read LFS 2018 quarters
#'
#' Reads and performs basic cleaning on the Labour Force Survey 2018 quarters.
#'
...
lfs_read_2018 <- function(
  ...
  data.q1 <- data.table::fread(
    paste0(path ,"/lfsp_jm18_eul.tab"), showProgress = FALSE,
    na.strings = c("NA", "", "-1", "-2", "-6", "-7", "-8", "-9", "-90", "-90.0", "N/A")
  )

  cat(crayon::green("\tApr - Jun..."))
  data.q2 <- data.table::fread(
    paste0(path ,"/lfsp_aj18_eul.tab"), showProgress = FALSE,
    na.strings = c("NA", "", "-1", "-2", "-6", "-7", "-8", "-9", "-90", "-90.0", "N/A")
  )
  ...
  data.q4 <- data.table::fread(
    paste0(path ,"/lfsp_od18_end_user_pwt18.tab"), showProgress = FALSE,
    na.strings = c("NA", "", "-1", "-2", "-6", "-7", "-8", "-9", "-90", "-90.0", "N/A")
  )
